...
(function(){
const CFG = (window.MIDRATA_REVIEW_CONFIG) || {};
...
/* C17 · library/features/review-widget/commentable-everything.md → anchorEmpty.
   A tag listed in MIDRATA_REVIEW_CONFIG.anchorEmpty bypasses the direct-text gate
...
function reviewer(){let n=store.get('midrata_reviewer');if(!n){n=(window.prompt(L.namePrompt,'')||'Anonymous').trim()||'Anonymous';store.set('midrata_reviewer',n);}return n;}
...
function localAdapter(){
  const KEY='midrata_review_comments'; let cb=null;
